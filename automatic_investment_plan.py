...
#%% US dollar montly/weekly/daily recording

import matplotlib.pyplot as plt
import numpy as np
import pandas as pd

# Loading the monthly data from excel file
dataframe = pd.read_excel('Investment_Monthly_Data.xlsx',names=('CIBC_USD','CIBC_CAD','BMO_CAD',
                                                                'Yearly_Return','Inflation_rate_US',
                                                                'PMI','PCE','PPI',
                                                                'yearly_return_rate'))

# Start from Feb 28th, 2022
CIBC_USD_monthly = dataframe['CIBC_USD'].dropna() # read the data and drop "not a number" nan
CIBC_CAD_monthly = dataframe['CIBC_CAD'].dropna()
BMO_CAD_monthly = dataframe['BMO_CAD'].dropna()
Yearly_return = dataframe['Yearly_Return'].dropna()
Inflation = dataframe['Inflation_rate_US'].dropna()
# Core_Inflation = 

# timelist_month = np.arange(len(CIBC_USD_monthly))
# timelist_month = np.arange(len(CIBC_CAD_monthly))
timelist_month = np.arange(len(BMO_CAD_monthly))
...
# plt.plot(timelist_month,CIBC_USD_monthly)
# plt.plot(timelist_month,CIBC_CAD_monthly)
plt.plot(timelist_month,BMO_CAD_monthly)
plt.xlabel('Months',fontsize=24)
plt.ylabel('US dollar [$\$$]',fontsize=24)
plt.tick_params(top=True,right=True,direction='in')
# plt.legend(handlelength=1.7, loc='upper center', bbox_to_anchor=(0.5, 1.20),
#            ncol=2,prop={'size': 16})
plt.show()

total = BMO_CAD_monthly[len(BMO_CAD_monthly)-1] + BMO_CAD_monthly[len(BMO_CAD_monthly)-1] + \
    CIBC_USD_monthly[len(CIBC_USD_monthly)-1] * 1.35
    
print("Total Investment Net Values ($):", total)
#%% Yearly return of SPX (S&P 500) since 1926 to present
...
fig = plt.figure(figsize=(6,4))
ax = fig.add_subplot(111)
plt.plot(Yearly_return)
plt.xlabel('Year')
plt.ylabel(r'Yearly return ($\%$)')
plt.tick_params(top=True,right=True,direction='in')
plt.show()

#%% CPI, inflation rate data from 2020 to 2024
...
for i in range(n):
    if month_counting > 12:
        month_counting = 1
        Date[i] = month_counting
        month_counting += 1
    else:
        Date[i] = month_counting
        month_counting += 1
"""
The Inflation rate spent 20 months to grow from 1.2% to 9.1%, start from Nov 31, 2020
    to June 30, 2022. If under 2% does not count, then it has increased 16 months. 
    If decreasing time of the inflation would take 16 months as well (probably less than
    16 months), then it will get to 2% around May, 2024. 
"""

size = 22
textsize = 18 # legends and (a)/(b)
plt.rcParams['lines.linewidth'] = 3
plt.rcParams.update({'font.size': size})
plt.rc('xtick', labelsize=size)
plt.rc('ytick', labelsize=size)
...
ax.plot(Inflation) 
ax.set_xticks(range(n),Date)
ax.locator_params(axis="x", nbins=15) 
ax.set_ylabel('Inflation rate ($\%$)')
ax.set_xlabel('Month')

# plt.savefig('2LOs_Mec_results'+filename,format='pdf',dpi=1200,bbox_inches='tight')
